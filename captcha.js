    ...
    return function(req, res, next){
		...
		var text = params.text || ('' + Math.random()).substr(3, params.codeLength);
      	...
      	for (i = 0; i < text.length; i++) {			
			ctx.setTransform(Math.random() * 0.5 + 1, Math.random() * 0.4, Math.random() * 0.4, Math.random() * 0.5 + 1, Math.floor(1*params.fontSize) * i + Math.floor(0.5*params.fontSize), Math.floor(1.25*params.fontSize));
